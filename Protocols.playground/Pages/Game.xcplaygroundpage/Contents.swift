        ...
        while !reachedGameEnd {
            for player in players where player.knockedOut == false {
                ...
                if diceRollSum == player.knockOutNumber {
                    ...
                    // Game Ends if all players are knocked out
                    if activePlayers.count == 0 {
                        reachedGameEnd = true
                        print("All players have been knoked out")
                    }
                } else {
                    // if not, add score to running score
                    player.score += diceRollSum
                    // game ends when a player reahed 100
                    if player.score >= 100 {
                        reachedGameEnd = true
                        print("Player \(player.id) has won with final \(player.score).")
